...
- [ ] 홈페이지
  ...
  - [ ] 인기 상품 섹션 (조회수 기준, 최대 6개 표시)
- [ ] 상품 목록 페이지
- [ ] 카테고리 필터링
- [ ] 상품 상세 페이지
  - **라우트**: `/products/[id]`
  - **레이아웃 구조**:
    - **상단 영역**: 상품 이름, 가격, 재고 상태 표시
    - **중단 영역**: 상품 설명, 카테고리 정보
    - **우측 고정 영역**: 장바구니 추가 UI (Phase 3에서 기능 구현, Phase 2에서는 UI 레이아웃만 준비)
    - **하단 영역**: 등록일 및 수정일 표시
  - **필수 표시 필드**:
    - 상품 이름 (`name`)
    - 가격 (`price`) - 천단위 구분 포맷팅
    - 재고 수량 (`stock_quantity`) - 재고 부족 시 경고 표시
    - 상품 설명 (`description`)
    - 카테고리 (`category`) - 한글 표시명 사용
    - 장바구니 추가 버튼 (Phase 2: UI만, Phase 3: 기능 연동)
    - 등록일 (`created_at`)
    - 수정일 (`updated_at`)
