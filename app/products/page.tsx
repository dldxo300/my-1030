...
export default function ProductsPage() {
  ...
  const updateURL = (category: Category | "all", sort: SortOption, page: number) => {
    console.log("🔗 [ProductsPage] URL 업데이트:", { category, sort, page });
  ...
  useEffect(() => {
    async function loadProducts() {
      console.group("🛍️ [ProductsPage] 상품 목록 로딩 시작");
      ...
      try {
        ...
        console.log(`✅ [ProductsPage] 로딩 성공`);
        ...
        console.error("❌ [ProductsPage] 에러:", errorMessage);
  ...
  const handleCategoryChange = (category: Category | "all") => {
    console.log(`🔄 [ProductsPage] 카테고리 변경: ${selectedCategory} → ${category}`);
  ...
  const handleSortChange = (sort: SortOption) => {
    console.log(`🔄 [ProductsPage] 정렬 변경: ${selectedSort} → ${sort}`);
  ...
  const handlePageChange = (page: number) => {
    console.log(`🔄 [ProductsPage] 페이지 변경: ${currentPage} → ${page}`);
    setCurrentPage(page);
